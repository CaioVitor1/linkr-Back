...
async function insertNewPost(id, comment, url, metaDados) {
  return connection.query(
    'INSERT INTO posts ("userId", comment, url, title, description, image) VALUES ($1, $2, $3, $4, $5, $6 )',
...
async function insertHashtag(tag, searchPostId) {
  return connection.query(
    'INSERT INTO hashtags (hashtag, "postId") VALUES ($1, $2)',
    [tag, searchPostId[0].id]
  );
}
...
async function updatingPost(updateComment, postId) {
  return connection.query(
    `UPDATE posts SET "comment" = '${updateComment}' WHERE id = $1`,
    [postId]
  );
}
